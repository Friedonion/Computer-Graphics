...
def CatmullRomSpline(p0,p1,p2,p3,t):
    t2 = t*t
    t3 = t2*t
    return 0.5*((p1*2)+(-p0+p2)*t+(p0*2-p1*5+p2*4-p3)*t2+(-p0+p1*3-p2*3+p3)*t3)

def display():
    global cameraIndex, cow2wld, cowList, count, animStart, animTime, anim, cowMax,animLoop
    ...
    if (not animStart) or (time == animTime):
        for cow in cowList:
            drawCow(cow, 0);									# Draw cow.
        drawCow(cow2wld, cursorOnCowBoundingBox)

    else:
        anim += (time-animTime) #delta time
        i = math.floor(anim)
        P = CatmullRomSpline(getTranslation(cowList[(i-1+cowMax)%cowMax]),getTranslation(cowList[i%cowMax]),getTranslation(cowList[(i+1)%cowMax]),getTranslation(cowList[(i+2)%cowMax]),anim-i)
        cp = np.eye(4)
        b = P - getTranslation(cow2wld) #direction vector
        u=normalize(b) #direction vector normal
        w=normalize(np.cross(u,np.array([0,1,0]))) #right vector
        v=np.cross(w,u) #new upvector
        cp[0:3,0:3] = np.transpose(np.array([u,v,w]))
        setTranslation(cp,P)
        cow2wld = cp
...
def onMouseButton(window,button, state, mods):
    global isDrag, V_DRAG, H_DRAG, count, pickInfo, cowList, animStart,animTime,anim,cow2wld
    GLFW_DOWN=1;
    GLFW_UP=0;
    x, y=glfw.get_cursor_pos(window)
    if button == glfw.MOUSE_BUTTON_LEFT and cursorOnCowBoundingBox and (not animStart):
        if state == GLFW_DOWN:
            if isDrag == H_DRAG:
                isDrag=V_DRAG;
                pickInfo.cowPickConfiguration = cow2wld
                pickInfo.cowPickPosition = transform(cow2wld,pickInfo.cowPickPositionLocal)
            print( "Left mouse down-click at %d %d\n" % (x,y))
            # start vertical dragging
        elif state == GLFW_UP:
            isDrag = H_DRAG
            if(count>0):
                cowList.append(cow2wld)
                pickInfo.cowPickConfiguration = cow2wld
                pickInfo.cowPickPosition = transform(cow2wld,pickInfo.cowPickPositionLocal)
                print(pickInfo.cowPickPosition)
                print( "Left mouse up\n")
                print("count is %d"%count)
            if(count == cowMax):
                isDrag = 0
                animTime = glfw.get_time()
                anim = 0
                animStart = 1
                cow2wld = cowList[0]
                i = 0
                for cow in cowList:
                    i=i+1
                    p = getTranslation(cow)
                    print(f'position of cow {i} : {p[0]},{p[1]},{p[2]}')
                print('Start anim')	
            count=count+1
...
def onMouseDrag(window, x, y):
    ...
    if isDrag: 
        ...
        if  isDrag==V_DRAG:
            ...
            T=np.eye(4)
            setTranslation(T, currentPos@P-pp.cowPickPosition@P)
